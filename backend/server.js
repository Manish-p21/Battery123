...
const batteryController = {
  ...
  getAllBatteries: async (req, res) => {
    try {
      const { category, brand, minPrice, maxPrice, capacity, sortBy, page = 1, limit = 500 } = req.query;
  ...
  searchBatteries: async (req, res) => {
    try {
      const { q, category, brand, minPrice, maxPrice, capacity, sortBy, page = 1, limit = 500 } = req.query;
