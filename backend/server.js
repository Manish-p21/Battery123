...
const batteryController = {
  ...
  getAllBatteries: async (req, res) => {
    try {
      const { category, brand, minPrice, maxPrice, capacity, sortBy, page = 1, limit = 1000 } = req.query;
  ...
  searchBatteries: async (req, res) => {
    try {
      const { q, category, brand, minPrice, maxPrice, capacity, sortBy, page = 1, limit = 1000 } = req.query;
...
// Middleware
app.use(cors({ origin: ["http://localhost:3000", "https://batteryboss.vercel.app"], credentials: true }));
